...
void CNavationCtrl::LocadStatusFromRC()
{
	...
	//Firse Level Phase
	STATE_ITEM item1;
	item1.dwID = PHASE_COMPONENTS;
	item1.bShow = TRUE;
	item1.nPhase = PHASE_PENDING;
	item1.nIDHeaderTitle = IDS_STRING_COMPONENTS;
	m_aryStates.Add(item1);

	STATE_ITEM item3;
	item3.dwID = PHASE_MESSAGE;
	item3.bShow = TRUE;
	item3.nPhase = PHASE_PENDING;
	item3.nIDHeaderTitle = IDS_STRING_MESSAGE;
	m_aryStates.Add(item3);

	STATE_ITEM item4;
	item4.dwID = PHASE_UNINSTALLATION;
	item4.bShow = TRUE;
	item4.nPhase = PHASE_PENDING;
	item4.nIDHeaderTitle = IDS_STRING_HEAD;
	m_aryStates.Add(item4);
	//////////////////////////////////////////////////////////////////////////
}
...
void CNavationCtrl::EnableStatusByHeaderTitleID(UINT nHeaderTitle, BOOL bEnalbe, BOOL bUpdate)
{
	BOOL blFind = FALSE;
	...
	{
		if (m_aryStates[i].nIDHeaderTitle == nHeaderTitle)
		{
	...
	for (j=0; j<nSize; j++)
	{
		...
		if (m_aryStates[j].bShow)
		{
			if (lWord == 0 && hByte > 0 && lByte == 0)
			{
				...
				m_ctrStatus.SetItemData(hRet, DWORD_PTR(&m_aryStates[j]));
				...
				for (i=0; i<nSize; i++)
				{
					if (m_aryStates[i].bShow)
					{
						...
						if (lWord2 == 0 && hByte == hByte2 && lByte2 > 0)
						{
							...
							HTREEITEM hSubRet = m_ctrStatus.InsertItem( &curTreeItem );
							...
							m_ctrStatus.SetItemData(hSubRet, DWORD_PTR(&m_aryStates[i]));
						}
					}
				}
		...
		if (m_aryStates[i].bShow)
		{
			...
			else//Need add a new item
			{
				...
				if (lWord == 0 && hByte > 0 && lByte == 0)
				{
					...
					m_ctrStatus.SetItemData(hRet, DWORD_PTR(&m_aryStates[i]));
...
void CNavationCtrl::RePositionControl()
{
	...
	m_ctrStatus.MoveWindow(rect, TRUE);
}
...
void CNavationCtrl::GetStatusFromFile(CString strFileName)
{
	...
	int nShowSize = theApp.SetupGetPrivateProfileInt(SECTION_STATUS, KEY_NUMBER, 0, strFileName);
	...
	for (int i=0; i<nShowSize; i++)
	{
		...
		int nValue = theApp.SetupGetPrivateProfileInt(SECTION_STATUS, strKey, 0, strFileName);
...
void CNavationCtrl::SetStatusToFile(CString strFileName)
{
	...
	theApp.SetupWritePrivateProfileString(SECTION_STATUS, KEY_NUMBER, strValue, strFileName);
	...
	for (INT_PTR i=0; i<nSize; i++)
	{
		...
		theApp.SetupWritePrivateProfileString(SECTION_STATUS, strKey, strValue, strFileName);
	}
}
