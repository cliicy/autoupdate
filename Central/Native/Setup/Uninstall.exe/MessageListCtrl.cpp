#include "stdafx.h"
#include "MessageListCtrl.h"
#include "memdc.h"
#include "resource.h"
#include <shlwapi.h>

#ifdef _DEBUG
#define new DEBUG_NEW
#endif

/////////////////////////////////////////////////////////////////////////////
// CMessageListCtrl

CMessageListCtrl::CMessageListCtrl()
{
	m_nTotalItems = 0;
	m_nListHeight = 0;
	m_nWndHeight = 0;
	m_nWndWidth = 0;

	m_nSelectedItem = NONE_SELECTED;

	m_penLight.CreatePen(PS_SOLID, 1, RGB(192, 192, 192));

	////Create the Font
	//LOGFONT lf = {0};
	//lf.lfHeight = -11;
	//lf.lfWeight = FW_NORMAL;
	//
	//_tcscpy_s(lf.lfFaceName, LF_FACESIZE, _T("Arial"));
	//m_font.CreateFontIndirect(&lf);

	m_clrBkSelectedItem = RGB(255, 255, 255);

	m_dwExtendedStyles = 0;

	m_bHasFocus = FALSE;

	m_ImageList.Create(IDB_HEAD_CHECKBOX, 16, 3, RGB(255, 0, 255));

	m_pImageList = NULL;
}

CMessageListCtrl::~CMessageListCtrl()
{
	DeleteAllItems();
}

BEGIN_MESSAGE_MAP(CMessageListCtrl, CWnd)
	//{{AFX_MSG_MAP(CMessageListCtrl)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_LBUTTONDOWN()
	ON_WM_VSCROLL()
	ON_WM_SIZE()
	ON_WM_SETFOCUS()
	ON_WM_KILLFOCUS()
	ON_WM_GETDLGCODE()
	ON_WM_KEYDOWN()
	ON_WM_RBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	ON_WM_DESTROY()
	ON_WM_SETCURSOR()
	ON_WM_MOUSEWHEEL()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()


/////////////////////////////////////////////////////////////////////////////
// CMessageListCtrl message handlers

BOOL CMessageListCtrl::Create(CWnd *pParent, CRect rc, UINT nID, DWORD dwStyle)
{
	//Get a New Class Name
	CString sWindowClassName = ::AfxRegisterWndClass(CS_DBLCLKS); 

	//Try to create it with default styles
	if (!CWnd::Create(sWindowClassName, _T("MESSAGELISTCTRL"), dwStyle, rc, pParent, nID))
	{
		return FALSE;
	}
	
	m_nControlID = nID;

	m_nWndWidth = rc.Width();
	m_nWndHeight = rc.Height();

	m_nListHeight = 0;

	//Set the scrolling info
	SCROLLINFO scrlinfo;
	scrlinfo.cbSize = sizeof(scrlinfo);

	scrlinfo.fMask = SIF_PAGE|SIF_RANGE;
	scrlinfo.nMax = 0;
	scrlinfo.nMin = 0;
	scrlinfo.nPage = 0;
	scrlinfo.nPos = 0;

	SetScrollInfo(SB_VERT, &scrlinfo);

	if (pParent)
	{
		CFont * pFont = pParent->GetFont();

		if (pFont)
		{
			SetFont(pFont);

			//Create the Font
			LOGFONT lf = {0};
			pFont->GetLogFont(&lf);
			m_font.CreateFontIndirect(&lf);
		}
	}

	return TRUE;
}

int CMessageListCtrl::InsertItem(CString sText, UINT uiImage, int nStyle, int nHeight)
{
	MESSAGELIST_ITEM *pItem = new MESSAGELIST_ITEM;
	pItem->sItemText = sText;
	pItem->nStyle = nStyle;
	pItem->nItemNo = m_nTotalItems;
	pItem->uiImage = uiImage;
	
	if (nHeight == AUTO)
	{
		pItem->nHeight = CalculateItemHeight(sText, nStyle, uiImage, m_nWndWidth);
		pItem->bHeightSpecified = FALSE;
	}
	else
	{
		pItem->nHeight = nHeight;
		pItem->bHeightSpecified = TRUE;
	}

	m_listItems.AddTail(pItem);
	
	m_nTotalItems++;

	m_nListHeight += pItem->nHeight;

	SCROLLINFO scrlinfo;
	scrlinfo.cbSize = sizeof(scrlinfo);

	scrlinfo.fMask = SIF_PAGE|SIF_RANGE;
	scrlinfo.nMax = m_nListHeight;
	scrlinfo.nMin = 0;
	scrlinfo.nPage = m_nWndHeight;
	scrlinfo.nPos = 0;

	SetScrollInfo(SB_VERT, &scrlinfo);

	m_mapItems.SetAt(pItem->nItemNo, pItem);

	return (m_nTotalItems - 1);
}

int CMessageListCtrl::CalculateItemHeight(CString sText, int nStyle, UINT uiImage,int nItemWidth)
{
	int nImageWidth = 0;
	int nPadding = ITEM_PADDING_LEFT;

	if (m_dwExtendedStyles & MESSAGELIST_STYLE_IMAGES)
	{
		if (m_pImageList)
		{
			if (m_pImageList->GetImageCount())
			{
				IMAGEINFO Info = {0};
				if (m_pImageList->GetImageInfo(uiImage,&Info))
				{
					nImageWidth = Info.rcImage.right - Info.rcImage.left;
					nImageWidth += (ITEM_IMAGE_PADDING_LEFT + ITEM_IMAGE_PADDING_RIGHT);
				}
			}
		}
	}

	if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
	{
		nPadding += ITEM_PADDING_CHECKBOX_LEFT + ITEM_CHECKBOX_WIDTH ;
	}
	
	if (nStyle == NORMAL_TEXT)
	{
		CDC *pDC = GetDC();
	
		CFont *pOldFont = pDC->SelectObject(&m_font);

		CRect rc;
		rc.SetRectEmpty();

		rc.left = 0;

		rc.right = nItemWidth - nPadding;

		rc.right -= nImageWidth;

		pDC->DrawText(sText,&rc,DT_WORDBREAK|DT_CALCRECT|DT_EXTERNALLEADING );

		pDC->SelectObject(pOldFont);

		ReleaseDC(pDC);

		return rc.Height() + ITEM_PADDING_BOTTOM + ITEM_PADDING_TOP;
	}
	else if (nStyle == HTML_TEXT)
	{
		CDC *pDC = GetDC();
		
		CDC memDC;
		memDC.CreateCompatibleDC(pDC);

		CFont *pOldFont = memDC.SelectObject(&m_font);

		int nWidth = 0;

		nWidth = nItemWidth - nPadding;
		nWidth -= nImageWidth;

		CRect rc(0, 0, nWidth, m_nWndHeight);
		
		int nHeight = DrawMessage(memDC.GetSafeHdc(), sText, sText.GetLength(), &rc, DT_LEFT|DT_CALCRECT|DT_WORDBREAK|DT_EXTERNALLEADING);

		memDC.SelectObject(pOldFont);

		ReleaseDC(pDC);

		return rc.Height() + ITEM_PADDING_BOTTOM + ITEM_PADDING_TOP;
	}
	else if (nStyle == SINGLE_LINE_TEXT)
	{
		CDC *pDC = GetDC();
	
		CFont *pOldFont = pDC->SelectObject(&m_font);

		CRect rc;
		rc.SetRectEmpty();

		rc.left = 0;

		rc.right = nItemWidth - nPadding;
		rc.right -= nImageWidth;

		pDC->DrawText(sText, &rc, DT_VCENTER|DT_CALCRECT|DT_SINGLELINE);

		pDC->SelectObject(pOldFont);

		ReleaseDC(pDC);

		return rc.Height() + ITEM_PADDING_BOTTOM + ITEM_PADDING_TOP;
	}

	return 0;
}

void CMessageListCtrl::DeleteAllItems()
{
	m_nListHeight = 0;
	m_nTotalItems = 0;

	POSITION pos = m_listItems.GetHeadPosition();

	for (int i = 0; i < m_listItems.GetCount(); i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);
		delete pItem;
	}

	m_listItems.RemoveAll();

	m_mapItems.RemoveAll();
}

LPARAM CMessageListCtrl::GetItemData(int nPos)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
		return pItem->lItemData;
	else
		return NULL;
}

void CMessageListCtrl::SetItemData(int nPos, LPARAM lItemData)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
	{
		pItem->lItemData = lItemData;
	}
}

void CMessageListCtrl::OnPaint() 
{
	CPaintDC dc(this);
	
	CMemExDC pDC(&dc);

	CFont *pOldFont = pDC->SelectObject(&m_font);

	CRect rcWnd;
	GetClientRect(&rcWnd);

	CRect rcItem = rcWnd;
	rcItem.bottom = 0;
	
	int nScrollPos = GetScrollPos(SB_VERT);

	rcItem.OffsetRect(0, -nScrollPos);

	POSITION pos = m_listItems.GetHeadPosition();
	for (int i = 0; i < m_listItems.GetCount(); i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);

		rcItem.bottom = rcItem.top + pItem->nHeight;
		
		if (i == m_nSelectedItem)
		{
			DrawItem(pDC, rcItem, pItem, TRUE);
		}
		else
		{
			DrawItem(pDC, rcItem, pItem, FALSE);
		}

		pItem->rcItem = rcItem;

		rcItem.OffsetRect(0, pItem->nHeight);
	}

	pDC->SelectObject(pOldFont);
}

BOOL CMessageListCtrl::OnEraseBkgnd(CDC* pDC) 
{
	return TRUE;
}

void CMessageListCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	SetFocus();

	NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

	pNMHDR->hdr.code = MESSAGELIST_LBUTTONDOWN;
	pNMHDR->hdr.hwndFrom = GetSafeHwnd();
	pNMHDR->hdr.idFrom = m_nControlID;

	pNMHDR->lItemData = 0;
	pNMHDR->nItemNo = -1;
	pNMHDR->sItemText.Empty();
	pNMHDR->bChecked = 0;

	GetParent()->SendMessage(WM_NOTIFY, m_nControlID, (LPARAM)pNMHDR);

	delete pNMHDR;

	BOOL bItemSelected = FALSE;
	m_nSelectedItem = NONE_SELECTED;

	POSITION pos = m_listItems.GetHeadPosition();

	for (int i = 0;i < m_listItems.GetCount();i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);
		if (pItem->rcItem.PtInRect(point))
		{
			if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
			{
				CPoint pt = pItem->rcItem.TopLeft();
				pt.x += 3;
				pt.y += pItem->rcItem.Height() / 2 - 8;
				

				CRect rcBox(pt, CPoint(pt.x + 15, pt.y + 15));
				
				if (rcBox.PtInRect(point))
				{
					pItem->bChecked = !pItem->bChecked;

					NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

					pNMHDR->hdr.code = MESSAGELIST_ITEMCHECKED;
					pNMHDR->hdr.hwndFrom = GetSafeHwnd();
					pNMHDR->hdr.idFrom = m_nControlID;

					pNMHDR->lItemData = pItem->lItemData;
					pNMHDR->nItemNo = pItem->nItemNo;
					pNMHDR->sItemText = pItem->sItemText;
					pNMHDR->bChecked = pItem->bChecked;

					GetParent()->SendMessage(WM_NOTIFY, m_nControlID, (LPARAM)pNMHDR);

					delete pNMHDR;
				}
			}

			NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

			pNMHDR->hdr.code = MESSAGELIST_SELECTIONCHANGED;
			pNMHDR->hdr.hwndFrom = GetSafeHwnd();
			pNMHDR->hdr.idFrom = m_nControlID;

			pNMHDR->lItemData = pItem->lItemData;
			pNMHDR->nItemNo = pItem->nItemNo;
			pNMHDR->sItemText = pItem->sItemText;
			pNMHDR->bChecked = pItem->bChecked;

			GetParent()->SendMessage(WM_NOTIFY,m_nControlID,(LPARAM)pNMHDR);

			delete pNMHDR;

			m_nSelectedItem = i;

			Invalidate(FALSE);

			bItemSelected = TRUE;

			break;
		}
	}

	if (!bItemSelected)
	{
		NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

		pNMHDR->hdr.code = MESSAGELIST_SELECTIONCHANGED;
		pNMHDR->hdr.hwndFrom = GetSafeHwnd();
		pNMHDR->hdr.idFrom = m_nControlID;

		pNMHDR->lItemData = 0;
		pNMHDR->nItemNo = NONE_SELECTED;
		pNMHDR->sItemText.Empty();
		
		GetParent()->SendMessage(WM_NOTIFY, m_nControlID, (LPARAM)pNMHDR);

		delete pNMHDR;
	}

	CWnd::OnLButtonDown(nFlags, point);
}

void CMessageListCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	int nScrollPos = GetScrollPos(SB_VERT);
	int nLimit = GetScrollLimit(SB_VERT);

	int nScroll = nLimit;

	int SCROLL_AMT_Y = 50;

	switch(nSBCode)
	{
		case SB_LINEUP:
		case SB_PAGEUP:
			if (nScrollPos <= 0)
			{
				return;
			}
			nScroll = min(nScrollPos, SCROLL_AMT_Y);
			SetScrollPos(SB_VERT, nScrollPos - nScroll);
			break;   
		case SB_LINEDOWN:
		case SB_PAGEDOWN:
			if (nScrollPos >= nLimit)
			{
				return;
			}
			nScroll = min(nScroll-nScrollPos, SCROLL_AMT_Y);
			SetScrollPos(SB_VERT, nScrollPos + nScroll);
		    break;
		case SB_THUMBPOSITION:
			{
				HWND hWndScroll;
				if ( pScrollBar == NULL )
					hWndScroll = m_hWnd;
				else
					hWndScroll = pScrollBar->m_hWnd;
				
				SCROLLINFO info;
				info.cbSize = sizeof(SCROLLINFO);
				info.fMask = SIF_TRACKPOS;
				::GetScrollInfo(hWndScroll, SB_VERT, &info);
				
				nPos = info.nTrackPos;

				int nScr = nScrollPos - nPos;
				
				SetScrollPos(SB_VERT,nPos);
			}
			break;
		case SB_THUMBTRACK:
			{
				HWND hWndScroll;
				if ( pScrollBar == NULL )
					hWndScroll = m_hWnd;
				else
					hWndScroll = pScrollBar->m_hWnd;
				
				SCROLLINFO info;
				info.cbSize = sizeof(SCROLLINFO);
				info.fMask = SIF_TRACKPOS;
				::GetScrollInfo(hWndScroll, SB_VERT, &info);
				
				nPos = info.nTrackPos;

				int nScr = nScrollPos - nPos;
				
				SetScrollPos(SB_VERT, nPos, FALSE);
			}
			break;
	}	
	
	Invalidate();

	UpdateWindow();

	CWnd::OnVScroll(nSBCode, nPos, pScrollBar);
}

void CMessageListCtrl::OnSize(UINT nType, int cx, int cy) 
{
	CWnd::OnSize(nType, cx, cy);
	
	CRect rc;
	GetClientRect(&rc);

	m_nWndHeight = rc.Height();
	m_nWndWidth = rc.Width();

	ReArrangeWholeLayout();
	
	SCROLLINFO scrlinfo;
	scrlinfo.cbSize = sizeof(scrlinfo);

	scrlinfo.fMask = SIF_PAGE|SIF_RANGE;
	scrlinfo.nMax = m_nListHeight;
	scrlinfo.nMin = 0;
	scrlinfo.nPage = m_nWndHeight;
	scrlinfo.nPos = 0;

	SetScrollInfo(SB_VERT, &scrlinfo);

	Invalidate(FALSE);
}

CRect CMessageListCtrl::GetItemRect(int nPos)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
	{
		return pItem->rcItem;
	}
	else
	{
		return NULL;
	}
}

BOOL CMessageListCtrl::OnMouseWheel( UINT nFlags, short zDelta, CPoint pt )
{
	int nVertScroll = GetScrollPos(SB_VERT);

	int maxpos = GetScrollLimit(SB_VERT);

	if (zDelta < 0)
	{
		int nNewPos = min(nVertScroll + 40 , maxpos);
		
		SetScrollPos(SB_VERT,nNewPos);

		UpdateWindow();
	}
	else
	{
		int nNewPos = max((nVertScroll - 40) , 0) ;
		
		SetScrollPos(SB_VERT, nNewPos);

		UpdateWindow();
	}

	Invalidate();

	return CWnd::OnMouseWheel(nFlags, zDelta, pt);
}

int CMessageListCtrl::GetSelectedItem()
{
	return m_nSelectedItem;
}

CString CMessageListCtrl::GetItemText(int nPos)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
	{
		return pItem->sItemText;
	}
	else
	{
		return _T("");
	}
}

void CMessageListCtrl::SetExtendedStyle(DWORD dwExStyle)
{
	m_dwExtendedStyles = dwExStyle;

	ReArrangeWholeLayout();

	SCROLLINFO scrlinfo;
	scrlinfo.cbSize = sizeof(scrlinfo);

	scrlinfo.fMask = SIF_PAGE|SIF_RANGE;
	scrlinfo.nMax = m_nListHeight;
	scrlinfo.nMin = 0;
	scrlinfo.nPage = m_nWndHeight;
	scrlinfo.nPos = 0;
	SetScrollInfo(SB_VERT,&scrlinfo);

	Invalidate(FALSE);
}

DWORD CMessageListCtrl::GetExtendedStyle()
{
	return m_dwExtendedStyles;
}

void CMessageListCtrl::OnSetFocus(CWnd* pOldWnd) 
{
	CWnd::OnSetFocus(pOldWnd);

	m_bHasFocus = TRUE;

	if (m_nSelectedItem != NONE_SELECTED)
	{
		InvalidateRect(GetItemRect(m_nSelectedItem));
	}
}

void CMessageListCtrl::OnKillFocus(CWnd* pNewWnd) 
{
	CWnd::OnKillFocus(pNewWnd);

	m_bHasFocus = FALSE;

	if (m_nSelectedItem != NONE_SELECTED)
	{
		InvalidateRect(GetItemRect(m_nSelectedItem));
	}
}

UINT CMessageListCtrl::OnGetDlgCode()
{
    return DLGC_WANTARROWS|DLGC_WANTCHARS;
}

void CMessageListCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (nChar == VK_UP)
	{
		if (m_nSelectedItem == NONE_SELECTED)
		{
			m_nSelectedItem = 0;
		}
		else
		{
			if (m_nSelectedItem > 0)
			{
				m_nSelectedItem --;
			}
		}

		EnsureVisible(m_nSelectedItem);

		Invalidate(FALSE);

		SendSelectionChangeNotification(m_nSelectedItem);
	}
	else if (nChar == VK_DOWN)
	{
		if (m_nSelectedItem == NONE_SELECTED)
		{
			m_nSelectedItem = m_nTotalItems - 1;
		}
		else
		{
			if (m_nSelectedItem < (m_nTotalItems - 1))
			{
				m_nSelectedItem ++;
			}
		}

		EnsureVisible(m_nSelectedItem);

		Invalidate(FALSE);

		SendSelectionChangeNotification(m_nSelectedItem);
	}
	else if (nChar == VK_SPACE)
	{
		if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
		{
			if (m_nSelectedItem != NONE_SELECTED)
			{
				if (GetItemCheck(m_nSelectedItem))
				{
					SetItemCheck(m_nSelectedItem, FALSE);
				}
				else
				{
					SetItemCheck(m_nSelectedItem);
				}

				SendCheckStateChangedNotification(m_nSelectedItem);
			}
		}
	}

	CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
}

void CMessageListCtrl::EnsureVisible(int nPos)
{
	int nScrollPos = GetScrollPos(SB_VERT);
	
	int nItemPos = 0;
	int nScrollAmount = 0;

	POSITION pos = m_listItems.GetHeadPosition();
	for (int i = 0;i < m_listItems.GetCount();i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);
		if (pItem->nItemNo == nPos)
		{
			if (nItemPos < nScrollPos)
			{
				nScrollAmount = nScrollPos - nItemPos;
			}
			else if ( (nItemPos + pItem->nHeight)> (nScrollPos + m_nWndHeight) )
			{
				nScrollAmount = (nScrollPos + m_nWndHeight) - nItemPos - pItem->nHeight;
			}
			break;
		}

		nItemPos += pItem->nHeight;
	}

	if (nScrollAmount)
	{
		SetScrollPos(SB_VERT, nScrollPos - nScrollAmount);

		Invalidate(FALSE);
	}
}

void CMessageListCtrl::SetItemCheck(int nPos,BOOL bCheck)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
	{
		pItem->bChecked = bCheck;
		InvalidateRect(pItem->rcItem,FALSE);
	}
}

BOOL CMessageListCtrl::GetItemCheck(int nPos)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
	{
		return pItem->bChecked;
	}
	else
	{
		return FALSE;
	}
}

void CMessageListCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

	pNMHDR->hdr.code = MESSAGELIST_RBUTTONDOWN;
	pNMHDR->hdr.hwndFrom = GetSafeHwnd();
	pNMHDR->hdr.idFrom = m_nControlID;

	pNMHDR->lItemData = 0;
	pNMHDR->nItemNo = -1;
	pNMHDR->sItemText = "";
	pNMHDR->bChecked = 0;

	GetParent()->SendMessage(WM_NOTIFY,m_nControlID,(LPARAM)pNMHDR);

	delete pNMHDR;

	CWnd::OnRButtonDown(nFlags, point);
}

void CMessageListCtrl::SetItemText(int nPos, CString sItemText,BOOL bCalculateHeight)
{
	MESSAGELIST_ITEM* pItem = GetInternalData(nPos);
	if (pItem)
	{
		pItem->sItemText = sItemText;
		if (bCalculateHeight)
		{
			pItem->bHeightSpecified = FALSE;

			ReArrangeWholeLayout();

			Invalidate(FALSE);
		}
		else
		{
			InvalidateRect(pItem->rcItem,FALSE);
		}
	}
}

BOOL CMessageListCtrl::DeleteItem(int nPos)
{
	POSITION pos = m_listItems.GetHeadPosition();
	for (int i = 0;i < m_listItems.GetCount();i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);
		if (pItem->nItemNo == nPos)
		{
			if (pos != NULL)
			{
				m_listItems.GetPrev(pos);
				m_listItems.RemoveAt(pos);
			}
			else
			{
				m_listItems.RemoveAt(m_listItems.GetTailPosition());
			}
			
			if (m_nSelectedItem == pItem->nItemNo)
			{
				m_nSelectedItem = NONE_SELECTED;
			}

			m_nListHeight -= pItem->nHeight;
			SCROLLINFO scrlinfo;
			scrlinfo.cbSize = sizeof(scrlinfo);

			scrlinfo.fMask = SIF_PAGE|SIF_RANGE;
			scrlinfo.nMax = m_nListHeight;
			scrlinfo.nMin = 0;
			scrlinfo.nPage = m_nWndHeight;
			scrlinfo.nPos = 0;
			SetScrollInfo(SB_VERT,&scrlinfo);	

			m_nTotalItems--;

			ReArrangeItems();

			Invalidate(FALSE);

			return TRUE;
		}
	}

	return FALSE;
}

void CMessageListCtrl::ReArrangeItems()
{
	m_mapItems.RemoveAll();
	POSITION pos = m_listItems.GetHeadPosition();
	for (int i = 0;i < m_listItems.GetCount();i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);
		pItem->nItemNo = i;
		SetInternalData(i, pItem);
	}
}

void CMessageListCtrl::ReArrangeWholeLayout()
{
	m_nTotalItems = 0;
	m_nListHeight = 0;
	m_mapItems.RemoveAll();

	POSITION pos = m_listItems.GetHeadPosition();
	for (int i = 0;i < m_listItems.GetCount();i++)
	{
		MESSAGELIST_ITEM *pItem = m_listItems.GetNext(pos);
		pItem->nItemNo = i;
		
		if (!pItem->bHeightSpecified)
		{
			pItem->nHeight = CalculateItemHeight(pItem->sItemText,pItem->nStyle,pItem->uiImage,m_nWndWidth);
		}
		
		m_nTotalItems ++;

		m_nListHeight += pItem->nHeight;

		SetInternalData(i, pItem);
	}
}

UINT CMessageListCtrl::GetImage(int nPos)
{
	MESSAGELIST_ITEM *pItem = GetInternalData(nPos);
	if (pItem)
	{
		return pItem->uiImage;
	}

	return 0;
}

void CMessageListCtrl::SetImage(int nPos, UINT uiImage)
{
	MESSAGELIST_ITEM *pItem = GetInternalData(nPos);
	if (pItem)
	{
		pItem->uiImage = uiImage;
		InvalidateRect(pItem->rcItem,FALSE);
	}
}

void CMessageListCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{	
	NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

	pNMHDR->hdr.code = MESSAGELIST_LBUTTONDBLCLICK;
	pNMHDR->hdr.hwndFrom = GetSafeHwnd();
	pNMHDR->hdr.idFrom = m_nControlID;

	pNMHDR->lItemData = 0;
	pNMHDR->nItemNo = -1;
	pNMHDR->sItemText.Empty();
	pNMHDR->bChecked = 0;

	GetParent()->SendMessage(WM_NOTIFY, m_nControlID, (LPARAM)pNMHDR);

	delete pNMHDR;

	CWnd::OnLButtonDblClk(nFlags, point);
}

void CMessageListCtrl::DrawItem(CDC *pDC, CRect rcItem, MESSAGELIST_ITEM *pItem, BOOL bSelected)
{
	CRect rcClipBox;
	pDC->GetClipBox(&rcClipBox);

	if (!IsRectVisible(rcClipBox,rcItem))
	{
		return;
	}

	COLORREF clrText = RGB(0,0,0);
	COLORREF clrOld;
	CRect rcImage(0,0,0,0);
		
	if (bSelected)
	{
		pDC->FillSolidRect(&rcItem,m_clrBkSelectedItem);
	}
	else
	{
		pDC->FillSolidRect(&rcItem, RGB(255, 255, 255));
	}
	
	if (m_dwExtendedStyles & MESSAGELIST_STYLE_GRIDLINES)
	{
		pDC->DrawEdge(&rcItem,BDR_SUNKENINNER,BF_LEFT|BF_BOTTOM|BF_FLAT);
	}
	
	CPoint ptCheckBox = rcItem.TopLeft();

	if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
	{
		ptCheckBox.x += ITEM_PADDING_CHECKBOX_LEFT;
		ptCheckBox.y += rcItem.Height() / 2 - 8;
		
		m_ImageList.Draw(pDC, pItem->bChecked ? 2:1, ptCheckBox, ILD_TRANSPARENT);

		ptCheckBox.x += ITEM_CHECKBOX_WIDTH;
	}
	
	if (m_dwExtendedStyles & MESSAGELIST_STYLE_IMAGES)
	{
		if (m_pImageList)
		{
			IMAGEINFO imgInfo = {0};
			m_pImageList->GetImageInfo(0,&imgInfo);
			rcImage = imgInfo.rcImage;
			
			CPoint pt = ptCheckBox;
			pt.x += ITEM_IMAGE_PADDING_LEFT;
			pt.y = rcItem.top;
			pt.y += rcItem.Height() / 2 - rcImage.Height()/2;
			
			m_pImageList->Draw(pDC,pItem->uiImage,pt,ILD_TRANSPARENT);			
		}
	}
	
	if (pItem->nStyle == NORMAL_TEXT)
	{
		clrOld = pDC->SetTextColor(clrText);
		
		CRect rc = rcItem;
		
		if (rcImage.Width())
		{
			rc.DeflateRect(rcImage.Width() + ITEM_IMAGE_PADDING_LEFT + ITEM_IMAGE_PADDING_RIGHT,0,0,0);
		}
		
		if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
		{
			rc.left += ITEM_PADDING_LEFT + ITEM_CHECKBOX_WIDTH;
		}
		else
		{
			rc.left += ITEM_PADDING_LEFT;
		}
		
		if (!pItem->bHeightSpecified)
			rc.top += ITEM_PADDING_TOP;
		
		pDC->DrawText(pItem->sItemText, pItem->sItemText.GetLength(), &rc,
			DT_LEFT|DT_WORDBREAK);
	}
	else if (pItem->nStyle == HTML_TEXT)
	{
		clrOld = pDC->SetTextColor(clrText);
		
		CRect rc = rcItem;
		if (rcImage.Width())
		{
			rc.DeflateRect(rcImage.Width() + ITEM_IMAGE_PADDING_LEFT + ITEM_IMAGE_PADDING_RIGHT,0,0,0);
		}

		if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
		{
			rc.left += ITEM_PADDING_LEFT + ITEM_CHECKBOX_WIDTH;
		}
		else
		{
			rc.left += ITEM_PADDING_LEFT;
		}
		
		if (!pItem->bHeightSpecified)
			rc.top += ITEM_PADDING_TOP;
		
		DrawMessage(pDC->GetSafeHdc(), pItem->sItemText, pItem->sItemText.GetLength(), &rc, DT_LEFT|DT_WORDBREAK);
	}
	else if (pItem->nStyle == SINGLE_LINE_TEXT)
	{
		clrOld = pDC->SetTextColor(clrText);
		
		CRect rc = rcItem;
		if (rcImage.Width())
		{
			rc.DeflateRect(rcImage.Width() + ITEM_IMAGE_PADDING_LEFT + ITEM_IMAGE_PADDING_RIGHT,0,0,0);
		}

		if (m_dwExtendedStyles & MESSAGELIST_STYLE_CHECKBOX)
		{
			rc.left += ITEM_PADDING_LEFT + ITEM_CHECKBOX_WIDTH;
		}
		else
		{
			rc.left += ITEM_PADDING_LEFT;
		}
		
		if (!pItem->bHeightSpecified)
			rc.top += ITEM_PADDING_TOP;
		
		TCHAR szBuffer[MAX_PATH*4];
		memset(szBuffer,0, sizeof(szBuffer));
		
		_tcscpy_s(szBuffer, _countof(szBuffer), pItem->sItemText);
		
		if (PathCompactPath(pDC->GetSafeHdc(), szBuffer, rc.Width()))
		{
			pDC->DrawText(szBuffer, _tcslen(szBuffer), &rc, DT_LEFT|DT_SINGLELINE|DT_VCENTER);
		}
		else
		{
			pDC->DrawText(pItem->sItemText, pItem->sItemText.GetLength(), &rc, DT_LEFT|DT_SINGLELINE|DT_VCENTER);
		}
	}
	
	pDC->SetTextColor(clrOld);
	
	if (m_bHasFocus && bSelected)
	{
		pDC->DrawFocusRect(&rcItem);
	}
}

void CMessageListCtrl::SendSelectionChangeNotification(int nPos)
{
	MESSAGELIST_ITEM *pItem = GetInternalData(nPos);
	if (pItem)
	{
		NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

		pNMHDR->hdr.code = MESSAGELIST_SELECTIONCHANGED;
		pNMHDR->hdr.hwndFrom = GetSafeHwnd();
		pNMHDR->hdr.idFrom = m_nControlID;

		pNMHDR->lItemData = pItem->lItemData;
		pNMHDR->nItemNo = pItem->nItemNo;
		pNMHDR->sItemText = pItem->sItemText;
		pNMHDR->bChecked = pItem->bChecked;

		GetParent()->SendMessage(WM_NOTIFY, m_nControlID, (LPARAM)pNMHDR);

		delete pNMHDR;
	}
}

void CMessageListCtrl::SendCheckStateChangedNotification(int nPos)
{
	MESSAGELIST_ITEM *pItem = GetInternalData(nPos);
	if (pItem)
	{
		NM_MESSAGELISTCTRL *pNMHDR = new NM_MESSAGELISTCTRL;

		pNMHDR->hdr.code = MESSAGELIST_ITEMCHECKED;
		pNMHDR->hdr.hwndFrom = GetSafeHwnd();
		pNMHDR->hdr.idFrom = m_nControlID;

		pNMHDR->lItemData = pItem->lItemData;
		pNMHDR->nItemNo = pItem->nItemNo;
		pNMHDR->sItemText = pItem->sItemText;
		pNMHDR->bChecked = pItem->bChecked;

		GetParent()->SendMessage(WM_NOTIFY, m_nControlID, (LPARAM) pNMHDR);

		delete pNMHDR;
	}
}

BOOL CMessageListCtrl::IsRectVisible(CRect rcClipBox, CRect rcItem)
{
	if (rcClipBox.top > rcItem.bottom || rcClipBox.bottom < rcItem.top) 
	{
		return FALSE;
	}

	return TRUE;
}

MESSAGELIST_ITEM * CMessageListCtrl::GetInternalData(int nPos)
{
	MESSAGELIST_ITEM *pData = NULL;
	m_mapItems.Lookup(nPos, pData);
	return pData;
}

void CMessageListCtrl::SetInternalData(int nPos, MESSAGELIST_ITEM *pData)
{
	m_mapItems.SetAt(nPos, pData);
}

void CMessageListCtrl::OnDestroy() 
{
	CWnd::OnDestroy();

	if (m_font.m_hObject != NULL)
		m_font.DeleteObject();
}

BOOL CMessageListCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	SetCursor(::LoadCursor(NULL, IDC_ARROW));

	return TRUE;
}
