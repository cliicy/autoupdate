...
public class CombiChart extends AreaChart {
	public CombiChart(ChartConfig config) {
		super(config);
	}
	
	@Override
	public ChartType getChartType() {
		return ChartType.Combi2D;
	}		
	...
	public String getChartDataXML(ChartModel chartModel) {
		...
		for (int sIndex = 0; sIndex < seriesCount; ++sIndex) {
			...
			if (!s.getRenderAs().isEmpty() && s.getRenderAs().equals(Series.TRENDLINES)) {
				for (DataPoint dp : s.getDataPoints()) {
					builder.append(" <trendLines> <line ");
					builder.append(this.addXmlAttribute("startvalue", dp.getValue()));
					builder.append(this.addXmlAttribute("color", dp.getColor().replaceAll("#", "")));
					builder.append(this.addXmlAttribute("displayvalue",	dp.getDisplayValue()));
					builder.append(this.addXmlAttribute("thickness", 2));
					builder.append(this.addXmlAttribute("showontop", 1));
					builder.append(this.addXmlAttribute("tooltext", dp.getTooltip()));
					builder.append(" valueOnRight='1'/> </trendLines>");
				}
			} else {
				...
				if (!s.getRenderAs().isEmpty()) {
					builder.append(this.addXmlAttribute("renderAs",	s.getRenderAs()));
				}
				...
				for (DataPoint dp : s.getDataPoints()) {
					...
					if (!dp.isNullValue()) {
						...
						if (displayValue != null)
							builder.append(this.addXmlAttribute("displayValue",
									displayValue));

						String tooltip = dp.getTooltip();
						if (tooltip != null && !tooltip.isEmpty()) {
							builder.append(this.addXmlAttribute("toolText",
									tooltip));
						}
