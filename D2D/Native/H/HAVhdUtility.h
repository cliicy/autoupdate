...
VHDCONV_API int GetIncDisks_MixMode(const wchar_t* pwszFullVhd, IDiskBmpReader* pDiffBitmap,
                                    ...
                                    IVDiskDiff** ppDiskDiff);
typedef int (*PFN_GetIncDisks_MixMode)(const wchar_t* pwszFullVhd, IDiskBmpReader* pDiffBitmap,
                                    const wchar_t* pwszIncVhd, D2D_ENCRYPTION_INFO* pEncInfo, 
                                    IVDiskDiff** ppDiskDiff);
#if 0
/*
...
typedef struct _t_create_parms
{
    ...
    VHD_TYPE        eType;
    __int64         llDiskSize;
    unsigned long   dwCreateDisp; // CREATE_ALWAYS or OPEN_ALWAYS
...
/**
* Create a VHD file writer, which will write a new VHD file or reopen and write data into 
* an existing VHD file.
*/
