/////////////////////////////////////////////////////////////////////////////
// CheyReg.h		CCheyRegistry Interface
...
//for VMware VM
#define REG_KEY_VMMEMCTL      _T("SYSTEM\\CurrentControlSet\\Services\\VMMEMCTL")
#define REG_KEY_VMMOUSE       _T("SYSTEM\\CurrentControlSet\\Services\\vmmouse")
#define REG_KEY_VMX_SVGA      _T("SYSTEM\\CurrentControlSet\\Services\\vmx_svga")
#define REG_KEY_VMXNET        _T("SYSTEM\\CurrentControlSet\\Services\\vmxnet")

//for Hper-V VM
#define REG_KEY_HYPERV        _T("SOFTWARE\\Microsoft\\Virtual Machine\\Guest\\Parameters")
#define REG_KEY_HYPERV_64     _T("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Virtualization")

//for VCB registry
#define REG_KEY_VMOUNT2                   _T("SYSTEM\\CurrentControlSet\\Services\\vmount2")
#define REG_KEY_UNINSTALL                 _T("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall")
#define REG_SUBKEY_VCB                    _T("VMware Consolidated Backup Framework")
#define REG_KEY_VALUE_DIPLAYNAME          _T("DisplayName")
#define REG_KEY_VALUE_INSTALLLOCATION     _T("InstallLocation")

//for Hyper-V Host
#define REG_KEY_HPERV_VHDSVC          _T("SYSTEM\\CurrentControlSet\\Services\\VHDSVC")

#ifdef _SETUPCLS_DLL 
class __declspec(dllexport) CCheyRegistry
#else
class __declspec(dllimport) CCheyRegistry
#endif
{
public:
...
protected:
	...
	static const CString m_sBABAgentDeployKey;
    ...
    void Construct(LPTSTR lpMachineName, BOOL bReadMode);


public:
    CCheyRegistry(BOOL		bAutoRead = TRUE, 
				  LPTSTR	lpLocalMachine = NULL);
    CCheyRegistry(CSettings	&rSettings,
				  BOOL		bAutoWrite = TRUE,
				  LPTSTR	lpLocalMachine = NULL);
	CCheyRegistry(BOOL	  bAutoRead,
	...
	BOOL GetProductInfoWithMask(LPCTSTR szProductName, ASProduct *pProduct, HKEY hRemoteKey = NULL);
	...
	BOOL  IsProductInstalledEx(LPCTSTR lpProductName);
	...
	BOOL CCheyRegistry::GetOldProductRootKey_ForUpg(LPTSTR lpBrightStorRootKey, HKEY hHKEYLocalMachine = NULL);
	BOOL  CCheyRegistry::IsProductInstalledWithoutBase(HKEY hHKEYLocalMachine = NULL);
	BOOL CCheyRegistry::IsBetaUpgProductInstalledWithoutBase(HKEY hHKEYLocalMachine /*= NULL*/);
	...
	BOOL IsAgentDeployPackageCopied();

	DWORD IsClusterSVCExisted(BOOL &bExisted);
...
public:
	...
	static BOOL RegKeyExist(HKEY hKey,LPCTSTR lpSubKey,LPCTSTR lpMachine=NULL);

	static BOOL RegKeyValueExist(HKEY hKey,LPCTSTR lpSubKey,LPCTSTR lpKeyValue,LPCTSTR lpMachine=NULL);

	////enum and check if the key value name exist , and key value data meet with lpNeedCheckKeyValueData , if lpNeedCheckKeyValueData=NULL, only check key value name
	static BOOL RegKeyEnumExist(HKEY hKey,LPCTSTR lpSubKey,LPCTSTR lpKeyValue,LPCTSTR lpNeedCheckKeyValueData,LPCTSTR lpMachine);

	//check if VMware VM is installed
	static BOOL IsVMWareVMInstalled(LPCTSTR lpMachine = NULL);

	//check if Hyper-V VM is installed
	static BOOL IsHyperVVMInstalled(LPCTSTR lpMachine = NULL);

	//check if VMware VCB(VMware Consolidated Backup Framework) is installed
	static BOOL IsVMVCBInstalled(LPCTSTR lpMachine = NULL);

	//check if Hyper-V Host is installed
	static BOOL IsHyperVHostInstalled(LPCTSTR lpMachine = NULL);
...
public:
	...
	// check whether the OS allow to install the SQL2008 Express 
	BOOL IsOS_AllowInstallSQL2008E(int& nOSVerMajor, int& nOSVerMinor);

	// determine whether the SQL2008 Express was already installed
	BOOL IsSQL2008Installed( BOOL& bPrevVersion );

	// determine whether the SQL Express acted as ASDB and return the ASDB instance version
	BOOL IsSQLExpressAsASDB( int& nMajVer, int& nMinVer, int& nBuild );

	// get special SQL instance version info version info
	BOOL Get32BitSQLInstanceInfo( PCTSTR lpctInstanceName, int& nMajVer, int& nMinVer, int& nBuild );

	// detect the SQL 7 installed or not!
	BOOL IsSQL7Installed();

	// detect the system reboot flag
	BOOL IsOSNeedReboot();
...
BOOL			GetAlertPath							(LPTSTR lpPath, LPDWORD lpdwSize, HKEY hHKEYLocalMachine /*= NULL*/);
...
BOOL IsRemote_64Bit_System( LPCTSTR lpMachine );
BOOL Is_Local_64Bit_System(LPCTSTR lpMachine);
#ifdef __cplusplus
}
#endif
