...
CUpdateSession::CUpdateSession(HANDLE hPipe, CPipeSessionSet* pSet)
	: CPipeSessionBase( hPipe, pSet )
{
}

CUpdateSession::~CUpdateSession()
{
}

DWORD CUpdateSession::Main()
{
...
LONG CUpdateSession::onInvalidCommand(DWORD cmd, void* pParamsIn)
{
	return m_pipe.sendAck(cmd, error_pipe_unsupported_cmd);
}

LONG CUpdateSession::onTriggerUpdate(void* pParamsIn)
{
	if (!pParamsIn)
	{
	...
	m_log.LogW(LL_INF, 0, L"%s:oooo Asked to trigger an update job for %d on %d bSvrSpecified=%d bHotfix=%d ,dwProdFor=%d,dwProdOn=%d", __WFUNCTION__, p->dwProdFor, p->dwProdOn, p->bSvrSpecified, p->bHotfix,p->dwProdFor,p->dwProdOn);
	LONG lError = 0;
	if (p->bSvrSpecified) {
		if (p->bHotfix )
...
LONG CUpdateSession::onQueryJobStatus(void* pParamsIn)
{
	if (!pParamsIn)
	{
...
LONG CUpdateSession::onCancelUpdateJob(void* pParamsIn)
{
	if (!pParamsIn)
	{
		m_log.LogW(LL_ERR, 0, L"%s: The update parameters is NULL", __WFUNCTION__);
